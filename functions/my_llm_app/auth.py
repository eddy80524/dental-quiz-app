...
def get_user_permission() -> bool:
    """
    セッション状態から権限情報を高速取得
    初回のみFirestoreから取得、その後はセッション状態から取得
    """
    # セッション状態に権限情報があればそれを返す（高速）
    if 'has_gakushi_permission' in st.session_state:
        return st.session_state['has_gakushi_permission']
    
    # セッション状態になければFirestoreから取得（初回のみ）
